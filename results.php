    ...
    <table class="table table-bordered table-striped align-middle">
      ...
      <tbody>
        ...
        <?php if (count($contestants) > 0): ?>
          ...
          <?php for ($i = 0; $i < count($contestants); $i++): ?>

            <?php
              $parts = explode(" (", $contestants[$i], 2);

              $contestantName = $parts[0];
              $party = isset($parts[1])
                  ? rtrim($parts[1], ")")
                  : "";
            ?>
...
document
  ...
  .addEventListener('click', () => switchChart('bar'));


document
  .getElementById('pieChartBtn')
  .addEventListener('click', () => switchChart('pie'));
